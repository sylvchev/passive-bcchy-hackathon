...
for subject in tqdm(range(1, n_subjects + 1), desc="subject"):
    ...
    for ppn, ppl in ens_ppl.items():
        X_idx_S1, X_idx_S2 = idx[:lsess["S1"]], idx[lsess["S1"]:lsess["S1"]+lsess["S2"]]
        ...
        # Train on sess 1, test on S3
        ppl1.fit(X_idx_S1, ysess["S1"])
        yp1 = ppl1.predict(X_idx_S3)
        ce, cm, cd = (yp1 == 0).sum(), (yp1 == 1).sum(), (yp1 == 2).sum()
        res = {"subject": sub, "pipeline": ppn, "train": "S1", "pred_easy": ce, "pred_med": cm, "pred_diff": cd}
        all_res.append(res)
        for est_n, est_p in ppl1.named_estimators_.items():
            ype1 = est_p.predict(X_idx_S3)
            ce, cm, cd = (ype1 == 0).sum(), (ype1 == 1).sum(), (ype1 == 2).sum()
            res = {
                "subject": sub,
                "train": "S1",
                "pred_easy": ce,
        ...
        # Train on sess 2, test on S3
        ppl2.fit(X_idx_S2, ysess["S2"])
        yp2 = ppl2.predict(X_idx_S3)
        ce, cm, cd = (yp2 == 0).sum(), (yp2 == 1).sum(), (yp2 == 2).sum()
        res = {"subject": sub, "train": "S2", "pipeline": ppn, "pred_easy": ce, "pred_med": cm, "pred_diff": cd}
        all_res.append(res)
        for est_n, est_p in ppl2.named_estimators_.items():
            ype2 = est_p.predict(X_idx_S3)
            ce, cm, cd = (ype2 == 0).sum(), (ype2 == 1).sum(), (ype2 == 2).sum()
            res = {
                "subject": sub,
                "train": "S2",
                "pred_easy": ce,
        ...
        res = {"subject": sub, "train": "S1+S2", "pipeline": ppn, "pred_easy": ce, "pred_med": cm, "pred_diff": cd}
        all_res.append(res)
        for est_n, est_p in ppl2.named_estimators_.items():
...
all_res.to_csv(data_dir + "classif_session3.csv")
